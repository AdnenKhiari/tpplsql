...
import { EntityArrayResponseType, FaculteService } from '../service/faculte.service';
...
export class FaculteComponent implements OnInit {
  ...
  trackFacno = (_index: number, item: IFaculte): number => this.faculteService.getFaculteIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.facultes = this.refineData(dataFromBody);
  }

  protected refineData(data: IFaculte[]): IFaculte[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IFaculte[] | null): IFaculte[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.faculteService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
